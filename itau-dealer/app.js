(() => {
  ...
  const els = {
    itauFile: $('itauFile'), itauText: $('itauText'), itauPasteStatus: $('itauPasteStatus'), companionFile: $('companionFile'), companionFolder: $('companionFolder'),
  ...
  async function parseItauFiles() {
    if (els.itauText && els.itauText.value.trim() && !findPrimaryFile()) {
      parseItauPastedText();
      return state.itauRows;
    }
    const primary = findPrimaryFile();
    if (!primary) throw new Error('Selecione o arquivo do Itaú ou cole os dados no campo.');
  ...
  function clearAll() {
    ...
    els.dealerText.value = ''; if (els.itauText) els.itauText.value = ''; els.searchInput.value = ''; els.mainMessage.textContent = '';
  ...
  if (els.itauText) {
    els.itauText.addEventListener('input', () => {
      if (els.itauText.value.trim()) setStatus(els.itauPasteStatus, 'Dados Itaú colados. Clique em Analisar conciliação.', 'muted');
      else setStatus(els.itauPasteStatus, 'Aguardando dados colados.');
    });
  }
